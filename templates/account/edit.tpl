							...
							<div class="d-flex flex-column gap-2" component="group/badge/list">
								...
								<div component="group/badge/item" class="d-flex gap-2 justify-content-between align-items-center" data-value="{./displayName}" data-selected="{./selected}">
									...
									<div class="d-flex gap-1">
										<button component="group/toggle/hide" type="button" class="btn btn-ghost btn-sm text-body {{{ if !./selected }}}hidden{{{ end }}}" title="{{tx("user:hide-group-title")}}"><i class="fa fa-fw fa-eye"></i></button>
										<button component="group/toggle/show" type="button" class="btn btn-ghost btn-sm text-body {{{ if ./selected }}}hidden{{{ end }}}" title="{{tx("user:show-group-title")}}"><i class="fa fa-fw fa-eye-slash"></i></button>
										{{{ if allowMultipleBadges }}}
										<button component="group/order/up" type="button" class="btn btn-ghost btn-sm text-body" title="{{tx("user:order-group-up")}}"><i class="fa fa-fw fa-chevron-up"></i></button>
										<button component="group/order/down" type="button" class="btn btn-ghost btn-sm text-body" title="{{tx("user:order-group-down")}}"><i class="fa fa-fw fa-chevron-down"></i></button>
										{{{ end }}}
									</div>
								</div>
								{{{ end }}}
			...
			<div class="col-12 col-xl-6">
				<div class="text-center">
					<ul class="list-group mb-3">
						...
						<a component="profile/change/picture" href="#" class="list-group-item">{{tx("user:change-picture")}}</a>
						<!-- ENDIF allowProfilePicture -->
						<!-- IF !username:disableEdit -->
						<a href="{config.relative_path}/user/{userslug}/edit/username" class="list-group-item">{{tx("user:change-username")}}</a>
						<!-- ENDIF !username:disableEdit -->
						<!-- IF !email:disableEdit -->
						<a href="{config.relative_path}/user/{userslug}/edit/email" class="list-group-item">{{tx("user:change-email")}}</a>
						<!-- ENDIF !email:disableEdit -->
						<!-- IF canChangePassword -->
						<a href="{config.relative_path}/user/{userslug}/edit/password" class="list-group-item">{{tx("user:change-password")}}</a>
						<!-- ENDIF canChangePassword -->
						{{{each editButtons}}}
						<a href="{config.relative_path}{editButtons.link}" class="list-group-item">{{tx(editButtons.text)}}</a>
						{{{end}}}
					</ul>
